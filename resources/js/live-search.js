  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      ${posts
        ...
        <img class="avatar-tiny" src="${post.user_owner.avatar}"> <strong>${post.title}</strong>
        <span class="text-muted small">by ${post.user_owner.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
